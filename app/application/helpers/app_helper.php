...
if(!function_exists('currency_format')) {
	function currency_format($number = '') {
		...
		$text = 'Rp. '.number_format($number, $decimal_length, $decimal_separator,$thausand_separator);
...
if (! function_exists('tanggal_indonesia')) {
    function tanggal_indonesia($tanggal, $cetak_hari = false)
    {
        if ($tanggal == '' || $tanggal == null) return '';
        ...
        $hasil = (int)$pecahkan[2] . ' ' . $bulan[(int)$pecahkan[1]] . ' ' . $pecahkan[0];

        if ($cetak_hari) {
            $num = date('N', strtotime($tanggal));
            return $hari[$num] . ', ' . $hasil;
        }
        return $hasil;
    }
}
 ...
 * contoh: 2025-09-09 17:25:25 -> 17:25:25
 *
 * @param string $datetime
 * @param bool $with_seconds (default true)
 * @return string
 */
if (! function_exists('jam_indonesia')) {
    function jam_indonesia($datetime, $with_seconds = true)
    {
...
{
    function format_bulan_tahun($tahun_bulan)
    {
        ...
        $bulan = substr($tahun_bulan, 5, 2); // Ambil bulan dari format yyyy-mm
        $tahun = substr($tahun_bulan, 0, 4); // Ambil tahun dari format yyyy-mm
        
        // Return bulan dan tahun dalam format Indonesia
