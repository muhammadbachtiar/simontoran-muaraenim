...
<section class="content-header">
    <h1>
        Register SPJ        <small>Edit Register SPJ</small>
    </h1>
    <ol class="breadcrumb">
            ...
            <div class="box box-warning">
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <?= form_open(base_url('administrator/register_spj/edit_save/'.$this->uri->segment(4)), [
                        ...
                        <div class="form-group ">
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="jenis_pengajuan" id="jenis_pengajuan" data-placeholder="Select Jenis Pengajuan" >
                                    ...
                                    <option <?=  $row->jenis_pengajuan ==  $register_spj->jenis_pengajuan ? 'selected' : ''; ?> value="<?= $row->jenis_pengajuan ?>"><?= $row->jenis_pengajuan; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                                <small class="info help-block">
                                </small>
                        ...
                        <div class="form-group ">
                            <label for="no_spm" class="col-sm-2 control-label">No Urut SPP-SPM
                            </label>
                            <div class="col-sm-4">
                                <input type="number" class="form-control" name="no_urut" id="no_urut" placeholder="No urut SPP-SPM">
                                <small class="info help-block">
                                </small>
                            </div>
                         ...
                         <div class="form-group ">
                            ...
                            <small class="info help-block">Masukkan No urut SPP-SPM dan tanggal SPP sebelum melakukan Generate
                            </small>
                            </div>
                                                        
                        </div>
                         <div class="form-group ">
                             <label for="tanggal_spp" class="col-sm-2 control-label">
                            </label>
                             <div class="col-sm-2">
                                <a class="btn btn-flat btn-warning btn_action" id="btn_generate" title="Generate No SPP-SPM">
                            <i class="fa fa-undo" ></i> Generate No SPP-SPM
                            </a>
                            </div>
                             </div>
                       ...
                       <div class="form-group ">
                            <label for="no_spm" class="col-sm-2 control-label">No Urut SP2D
                            </label>
                            <div class="col-sm-4">
                                <input type="number" class="form-control" name="no_urut_sp2d" id="no_urut_sp2d" placeholder="No urut SP2D">
                                <small class="info help-block">
                                </small>
                            </div>
                        ...
                        <div class="form-group ">
                             ...
                             <div class="col-sm-2">
                                <a class="btn btn-flat btn-warning btn_action" id="btn_generate_sp2d" title="Generate No SP2D">
                            <i class="fa fa-undo" ></i> Generate No SP2D
                            </a>
                            </div>
                             </div>
...
function generateNumbers() {
        ...
        if (noUrut === '') {
            swal('Error', 'Nomor urut tidak boleh kosong', 'error');
            return;
        }

        if (tglSPP === '')  {
            swal('Error', 'Tanggal SPP tidak boleh kosong', 'error');
            return;
        }
        
        else if (tglSPP === '2000-0m-dd')  {
            swal('Error', 'Tanggal SPP tidak boleh kosong', 'error');
            return;
        }

        var dateParts = tglSPP.split('-');

        if (dateParts.length !== 3) {
            alert('Format tanggal SPP tidak valid');
            return;
        }

        var tahun = dateParts[0];
        var bulan = parseInt(dateParts[1], 10);
        var baseFormatSPP = '35.05/02.0/0000/'+jenis_pengajuan+'/4.01.0.00.0.00.01.0000/P/'+bulan+'/'+tahun;
        var baseFormatSPM = '35.05/03.0/0000/'+jenis_pengajuan+'/4.01.0.00.0.00.01.0000/M/'+bulan+'/'+tahun;

        var formattedNoUrutSPP = String(noUrut).padStart(4, '0');
        var formattedNoUrutSPM = String(parseInt(noUrut) + 1).padStart(4, '0');

        var noSPP = baseFormatSPP.replace('0000', formattedNoUrutSPP);
        var noSPM = baseFormatSPM.replace('0000', formattedNoUrutSPM);

        $('#no_spp').val(noSPP);
        $('#no_spm').val(noSPM);
        $('#tanggal_spm').val(tglSPP);
        
    }
    
function generateNumbers_sp2d() {
        var noUrut = $('#no_urut_sp2d').val();
        var tglSPP = $('#tanggal_sp2d').val();
        var jenis_pengajuan = $('#jenis_pengajuan').val();
        ...
        if (tglSPP === '')  {
            swal('Error', 'Tanggal SP2D tidak boleh kosong', 'error');
            return;
        }
        ...
        if (dateParts.length !== 3) {
            alert('Format tanggal SP2D tidak valid');
            return;
        }
        ...
        var baseFormatSP2D ='35.05/04.0/0000/'+jenis_pengajuan+'/4.01.0.00.0.00.01.0000/M/'+bulan+'/'+tahun;
        

        var formattedNoUrutSPP = String(noUrut).padStart(4, '0');
       

        var no_sp2d = baseFormatSP2D.replace('0000', formattedNoUrutSPP);
    ...
    $('#btn_generate_sp2d').click(function(){
        
            generateNumbers_sp2d();
        });
